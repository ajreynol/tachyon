...
Writes index.html, summary.json and benchmarks.csv (rewritten whole) under
--output, plus plots.py's PDFs with --pdf. Never modifies inputs or this
repository's documentation or recorded evidence.
"""
import argparse
import csv
import fnmatch
...
def write_report(output, records, config, warnings, source):
    ...
    html = template.replace("__PROFILE_DATA__", encoded).replace("<!doctype html>",
        "<!doctype html>\n<!-- Generated by stats_profiler/profile.py; rewritten whole. -->", 1)
...
def main(argv=None):
    ...
    try:
        if args.pdf and not 0 < args.max_share <= 100:
